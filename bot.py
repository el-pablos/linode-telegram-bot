...
AUTO_APPEND_CREATED_VPS = os.getenv("AUTO_APPEND_CREATED_VPS", "true").lower() in {"1", "true", "yes", "on"}
GEN3_VPS_FILE = os.getenv("GEN3_VPS_FILE", "/root/work/gen3-vps/vps.json")
GEN3_VPS_USERNAME = os.getenv("GEN3_VPS_USERNAME", "root")
AUDIT_LOG_FILE = os.getenv("AUDIT_LOG_FILE", os.path.join(BASE_DIR, "logs", "audit.jsonl"))
...
def load_accounts() -> dict[str, dict[str, Any]]:
    ...
    if LINODE_API_TOKEN and not accounts:
        ...
        accounts[name] = {"name": name, "username": LINODE_ACCOUNT_NAME, "token": LINODE_API_TOKEN, "enabled": True}
    return accounts
...
async def images_catalog(force: bool = False) -> list[dict[str, Any]]:
    ...
    popular = ["ubuntu24.04", "ubuntu22.04", "debian12", "debian11", "almalinux9", "rocky9", "centos-stream9"]
...
async def account_health(account_name: str, force: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            limits = await linode_request("GET", "/account/limits", account=account_name)
        except Exception as e:
            limits = {}
            health["last_error"] = f"limits unknown: {e}"
        instances = await get_paginated("/linode/instances", account=account_name)
        used = len(instances)
        limit = limits.get("linodes") or limits.get("linode_instances") or limits.get("linode")
        ...
        try:
            ...
            health["notifications"] = [n.get("label") or n.get("type") for n in notif[:5]]
        except Exception:
            pass
...
async def accounts_health(scope: str = "all", force: bool = False) -> list[dict[str, Any]]:
    if scope != "all":
...
def health_icon(h: dict[str, Any]) -> str:
    return {"ok": "🟢", "warn": "🟡", "block": "🔴", "error": "🔴"}.get(h.get("status"), "⚪")
...
async def health_text(scope: str = "all", force: bool = False) -> str:
    ...
    for h in hs:
        ...
        note = f" note=<code>{esc(h.get('last_error'))}</code>" if h.get("last_error") else ""
        lines.append(
...
def pick_accounts_for_plan(plan: dict[str, Any], healthy_accounts: list[str] | None = None) -> list[dict[str, Any]]:
...
async def preflight_create(plan: dict[str, Any], force: bool = False) -> dict[str, Any]:
    ...
    if plan.get("account_mode") == "smart":
        ...
        ok_names = [h["account"] for h in hs if h.get("can_create") or h.get("linodes_remaining") is None]
        if not ok_names:
            blockers.append("Tidak ada account sehat untuk smart create")
        return {"ok": not blockers, "warnings": warnings, "blockers": blockers, "health": hs}
    ...
    for account_name, need in counts.items():
        ...
        if not h.get("token_ok"):
            ...
            blockers.append(f"{account_name}: quota kurang need={need} remaining={remaining}")
...
def build_plan_from_options(args: dict[str, str], user_id: int | None = None, defaults: dict[str, Any] | None = None) -> dict[str, Any]:
    defaults = defaults or (get_wizard(user_id) if user_id else default_wizard())
    account_mode, account = parse_account_arg(args.get("account") or args.get("api"), defaults)
    ...
    random_region = parse_bool(args.get("random_region", str(defaults.get("random_region", False)))) or region_low in {"random", "rand", "acak", "auto"}
    region_mode = "auto" if region_low == "auto" else ("random" if random_region else "specific")
    if random_region:
        region = "random"
    smart_create = parse_bool(args.get("smart", str(defaults.get("smart_create", False)))) or account_mode == "smart"
    type_id = args.get("type", defaults.get("type", DEFAULT_TYPE))
    image = args.get("image", defaults.get("image", DEFAULT_IMAGE))
    label_prefix = clean_label(args.get("label", args.get("label_prefix", defaults.get("label_prefix", "linode-bot"))))
    root_pass = args.get("root_pass") or defaults.get("root_pass") or gen_password()
    tags = [x.strip() for x in args.get("tags", ",".join(defaults.get("tags") or ["telegram-bot"])).split(",") if x.strip()]
    group = sanitize_group_name(args.get("group", defaults.get("group", ""))) if args.get("group", defaults.get("group", "")) else ""
    if group:
        gt = group_tag(group)
        if gt not in tags:
            tags.append(gt)
    plan = {
        ...
        "tags": tags,
        "group": group,
        "backups_enabled": parse_bool(args.get("backups", str(defaults.get("backups_enabled", False)))),
        "private_ip": parse_bool(args.get("private_ip", str(defaults.get("private_ip", False)))),
        "save_vps": parse_bool(args.get("save_vps", str(defaults.get("save_vps", AUTO_APPEND_CREATED_VPS)))),
    }
    return plan
...
def wizard_keyboard(state: dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton(f"🎲 Random region {rr}", callback_data="wiz:toggle:random"), InlineKeyboardButton(f"🧠 Smart {smart}", callback_data="wiz:toggle:smart")],
        [InlineKeyboardButton("📦 Plan/Type", callback_data="wiz:pick:type:0"), InlineKeyboardButton("💿 Image", callback_data="wiz:pick:image:0")],
        [InlineKeyboardButton("🔢 Count", callback_data="wiz:pick:count:0"), InlineKeyboardButton("🏷 Label", callback_data="wiz:ask:label")],
        [InlineKeyboardButton("👥 Group", callback_data="wiz:ask:group"), InlineKeyboardButton("🏷 Tags", callback_data="wiz:ask:tags")],
        ...
        [InlineKeyboardButton(f"📥 Save JSON {save}", callback_data="wiz:toggle:save"), InlineKeyboardButton("🔑 Root pass", callback_data="wiz:ask:root_pass")],
        [InlineKeyboardButton("🩺 Quota", callback_data="cmd:health"), InlineKeyboardButton("🔄 Scrape/Refresh", callback_data="wiz:refresh")],
...
async def picker_keyboard(kind: str, page: int) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if kind == "account":
        ...
        rows = [
            [InlineKeyboardButton("🧠 SMART BEST ACCOUNT", callback_data="wiz:set:account_mode:smart")],
            [InlineKeyboardButton("🌐 SPREAD/ALL ACCOUNTS", callback_data="wiz:set:account_mode:spread")],
    ...
    if kind == "region":
        ...
        start = page * PAGE_SIZE
        shown = items[start : start + PAGE_SIZE]
        rows = [[InlineKeyboardButton(f"{r.get('id')} - {short(r.get('label'), 26)}", callback_data=f"wiz:set:region:{r.get('id')}")] for r in shown]
        rows.insert(0, [InlineKeyboardButton("🎲 RANDOM REGION PER VPS", callback_data="wiz:set:region:random")])
        rows.insert(1, [InlineKeyboardButton("🧠 AUTO REGION SMART", callback_data="wiz:set:region:auto")])
...
def confirm_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("Confirm create", callback_data="confirm_create"), InlineKeyboardButton("Cancel", callback_data="cancel_create")]])


def main_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("🚀 Visual Create", callback_data="wiz:menu"), InlineKeyboardButton("📋 Dashboard", callback_data="dash:menu")],
            [InlineKeyboardButton("👤 Accounts", callback_data="cmd:accounts"), InlineKeyboardButton("🩺 Health/Quota", callback_data="cmd:health")],
            [InlineKeyboardButton("👥 Groups", callback_data="cmd:groups"), InlineKeyboardButton("📤 Export", callback_data="cmd:export")],
...
async def normalize_instance(x: dict[str, Any], account: str, root_pass: str | None = None) -> dict[str, Any]:
    ipv4 = x.get("ipv4") or []
...
async def collect_instances(account_scope: str = "all") -> list[dict[str, Any]]:
    ...
    accounts = enabled_accounts() if account_scope == "all" else [get_account(account_scope)]
    for acct in accounts:
        try:
            ...
            rows.append({"account": acct["name"], "id": None, "label": f"ERROR: {e}", "status": "error", "ipv4": [], "tags": []})
    return rows
...
def default_dashboard() -> dict[str, Any]:
    return {"account": "all", "region": "all", "status": "all", "search": "", "page": 0, "selected": set(), "page_items": []}
...
def filter_dashboard_rows(rows: list[dict[str, Any]], state: dict[str, Any]) -> list[dict[str, Any]]:
    out = []
    q = str(state.get("search") or "").lower()
    for r in rows:
        if not r.get("id"):
            continue
        if state.get("region") not in {None, "all"} and r.get("region") != state.get("region"):
            continue
        if state.get("status") not in {None, "all"} and r.get("status") != state.get("status"):
            continue
        if q and q not in str(r.get("label", "")).lower() and q not in str(r.get("id", "")).lower() and q not in first_ip(r):
            continue
        out.append(r)
    return out
...
async def render_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    rows = filter_dashboard_rows(await collect_instances(state.get("account", "all")), state)
    ...
    for i, r in enumerate(shown):
        ...
        lines.append(f"{mark} {i+1}. <code>{esc(r['account'])}</code> <code>{esc(r['id'])}</code> <b>{esc(r['label'])}</b> {esc(r['status'])} {esc(first_ip(r))}")
    rows_btn: list[list[InlineKeyboardButton]] = []
    for i, r in enumerate(shown):
        mark = "✅" if row_key(r) in selected else "⬜"
        rows_btn.append([
            InlineKeyboardButton(f"{mark} {short(r.get('label'), 18)}", callback_data=f"dash:toggle:{i}"),
    ...
    rows_btn.append(nav)
    rows_btn += [
        [InlineKeyboardButton("👤 Account", callback_data="dash:filter:account"), InlineKeyboardButton("🌍 Region", callback_data="dash:ask:region"), InlineKeyboardButton("🟢 Status", callback_data="dash:filter:status")],
        [InlineKeyboardButton("🔎 Search", callback_data="dash:ask:search"), InlineKeyboardButton("☑️ Select page", callback_data="dash:selectpage"), InlineKeyboardButton("🧹 Clear", callback_data="dash:clear")],
        [InlineKeyboardButton("🗑 Delete selected", callback_data="dash:bulk:delete"), InlineKeyboardButton("🔁 Reboot selected", callback_data="dash:bulk:reboot")],
        [InlineKeyboardButton("📦 Resize selected", callback_data="dash:bulk:resize"), InlineKeyboardButton("📤 Export selected", callback_data="dash:bulk:export")],
        [InlineKeyboardButton("☢️ Delete ALL filtered", callback_data="dash:all:delete"), InlineKeyboardButton("⬅️ Main", callback_data="cmd:start")],
    ]
...
def make_targets(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [{"account": r["account"], "id": int(r["id"]), "label": r.get("label"), "region": r.get("region"), "type": r.get("type"), "ipv4": r.get("ipv4") or []} for r in rows if r.get("id")]


def create_pending_action(uid: int, action: str, targets: list[dict[str, Any]], meta: dict[str, Any] | None = None) -> str:
    action_id = uuid.uuid4().hex[:10]
    PENDING_ACTIONS[action_id] = {"uid": uid, "action": action, "targets": targets, "meta": meta or {}, "created_at": time.time()}
...
def pending_action_text(action_id: str) -> str:
    ...
    for t in targets[:20]:
        lines.append(f"- <code>{esc(t['account'])}</code> <code>{esc(t['id'])}</code> {esc(t.get('label'))}")
    if len(targets) > 20:
...
def pending_action_keyboard(action_id: str) -> InlineKeyboardMarkup:
    ...
    if phrase:
        return InlineKeyboardMarkup([[InlineKeyboardButton("Cancel", callback_data=f"act:cancel:{action_id}")]])
    return InlineKeyboardMarkup([[InlineKeyboardButton("Confirm", callback_data=f"act:confirm:{action_id}"), InlineKeyboardButton("Cancel", callback_data=f"act:cancel:{action_id}")]])


async def execute_pending_action(uid: int, action_id: str, update: Update | None = None, message=None) -> None:
    ...
    ok: list[str] = []
    err: list[str] = []
    audit_event(f"linode.{action}.confirmed", "confirmed", update=update, request={"targets": p["targets"], "meta": p.get("meta")})
    for t in p["targets"]:
        try:
            if action == "delete":
                await linode_request("DELETE", f"/linode/instances/{t['id']}", account=t["account"])
            elif action == "reboot":
                await linode_request("POST", f"/linode/instances/{t['id']}/reboot", account=t["account"])
            elif action == "resize":
                new_type = p.get("meta", {}).get("type")
                if not new_type:
                    raise BotError("resize type kosong")
                await linode_request("POST", f"/linode/instances/{t['id']}/resize", account=t["account"], json={"type": new_type, "allow_auto_disk_resize": True})
            else:
                raise BotError(f"action invalid: {action}")
            ok.append(f"✅ {t['account']}:{t['id']} {t.get('label')}")
            audit_event(f"linode.{action}.success", "success", update=update, account=t["account"], resource={"type": "linode", "id": t["id"], "label": t.get("label")})
        except Exception as e:
            err.append(f"❌ {t['account']}:{t['id']} {e}")
            audit_event(f"linode.{action}.failed", "failed", update=update, account=t.get("account"), resource={"type": "linode", "id": t.get("id"), "label": t.get("label")}, error=e)
...
async def create_linode_once(plan: dict[str, Any], index: int, account: str, region: str) -> dict[str, Any]:
    ...
    res = await linode_request("POST", "/linode/instances", account=account, json=payload)
    res["_chosen_region"] = region
    res["_account"] = account
    return res


async def smart_create_one(plan: dict[str, Any], index: int) -> tuple[dict[str, Any] | None, list[str]]:
    attempts: list[str] = []
    hs = await accounts_health("all")
    healthy = [h["account"] for h in hs if h.get("can_create") or h.get("linodes_remaining") is None]
...
async def fetch_instance_until_ip(account: str, linode_id: int, tries: int = 3) -> dict[str, Any] | None:
    for _ in range(tries):
        try:
            x = await linode_request("GET", f"/linode/instances/{linode_id}", account=account)
            if x.get("ipv4"):
                return x
        except Exception:
            pass
    ...
    for r in rows:
        ...
        for ip in r.get("ipv4") or []:
            if not ip or ip in seen:
                skipped += 1
                continue
            data.append({"host": ip, "username": GEN3_VPS_USERNAME, "password": password})
            seen.add(ip)
            added += 1
...
async def do_create(user_id: int, update: Update | None = None, message=None) -> None:
    ...
    if not report.get("ok") and not plan.get("smart_create"):
        if target:
            await target.reply_html("<b>Create diblock preflight</b>\n" + preflight_summary(report))
    ...
    if plan.get("smart_create") or plan.get("account_mode") == "smart":
        for i in range(1, plan["count"] + 1):
            ...
            if res:
                ...
                errors.append(f"smart-{i:02d}: gagal setelah attempts: {' | '.join(attempts[-5:])}")
    else:
        accounts = pick_accounts_for_plan(plan)
        regs = await region_pool(plan)
        for i in range(1, plan["count"] + 1):
            account = accounts[i - 1]["name"]
            region = random.choice(regs) if plan.get("random_region") else plan["region"]
            try:
                ...
                audit_event("linode.create.failed", "failed", update=update, account=account, request={"index": i, "plan": plan}, error=e)
    for x in created_raw:
        ...
        audit_event("linode.create.success", "success", update=update, account=account, resource={"type": "linode", "id": x.get("id"), "label": x.get("label")}, request=plan)
    ...
    if created_norm and plan.get("save_vps"):
        try:
            ...
            lines.append(f"vps.json: added={save_info['added']} skipped={save_info['skipped']} path=<code>{esc(save_info['path'])}</code>")
            audit_event("vpsjson.append.success", "success", update=update, request=save_info)
        except Exception as e:
            lines.append(f"vps.json error: <code>{esc(e)}</code>")
            audit_event("vpsjson.append.failed", "failed", update=update, error=e)
    if created_norm:
        lines.append(f"root_pass: <code>{esc(plan['root_pass'])}</code>")
        lines.append("Simpan sekarang. Password tidak disimpan kecuali save_vps_json ON.")
    for err in errors:
        lines.append(f"❌ {esc(err)}")
    kb = InlineKeyboardMarkup(
        [[InlineKeyboardButton("📤 Export new JSON", callback_data="export:new:json"), InlineKeyboardButton("📄 Export new TXT", callback_data="export:new:txt")], [InlineKeyboardButton("📋 Dashboard", callback_data="dash:menu")]]
    )
    if target:
...
def export_rows_json(rows: list[dict[str, Any]], include_secret: bool = False) -> str:
    data = rows if include_secret else [{k: v for k, v in r.items() if k != "password"} for r in rows]
    return json.dumps(data, indent=2, ensure_ascii=False) + "\n"


def export_rows_txt(rows: list[dict[str, Any]], include_secret: bool = False, wa: bool = False) -> str:
    lines: list[str] = []
    for r in rows:
        pwd = r.get("password") if include_secret else None
        if wa:
            lines.append(f"✅ VPS {r.get('label')}\nIP: {first_ip(r)}\nUser: {r.get('username', 'root')}\nPass: {pwd or '-'}\nAccount: {r.get('account')}\nRegion: {r.get('region')}\nType: {r.get('type')}")
        else:
            lines.append(f"{r.get('label')} | {first_ip(r)} | {r.get('username','root')} | {pwd or '-'} | {r.get('account')} | {r.get('region')} | {r.get('type')} | {r.get('status')}")
    return "\n".join(lines) + ("\n" if lines else "")


def export_rows(rows: list[dict[str, Any]], fmt: str, include_secret: bool = False) -> tuple[str, str]:
    fmt = fmt.lower()
    if fmt == "json":
...
async def send_export(target_message, rows: list[dict[str, Any]], fmt: str = "txt", title: str = "vps-export", include_secret: bool = False) -> None:
    ...
    with open(path, "rb") as f:
        await target_message.reply_document(document=f, filename=filename, caption=f"Export {len(rows)} VPS")
    audit_event("vps.export.success", "success", request={"title": title, "fmt": fmt, "rows": len(rows), "include_secret": include_secret})
...
async def accounts_text(uid: int | None = None) -> str:
    ...
    for a in ACCOUNTS.values():
        ...
        lines.append(f"<code>{esc(a['name'])}</code> [{status}] user=<code>{esc(a.get('username', '-'))}</code> token=<code>{esc(mask_token(a.get('token', '')))}</code>")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_html(
        ...
        "Cmd: /accounts /quota /dashboard /groups /export /list /delete /regions /types /images /refresh /whoami",
...
async def account_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        audit_event("account.selected", "success", update=update, account=account, request={"mode": mode})
...
async def delete_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if len(parts) == 2 and parts[1].isdigit():
            state = get_wizard(update.effective_user.id)
            if state.get("account_mode") != "specific":
                raise BotError("Active account random/roundrobin/spread/smart. Pakai /delete ACCOUNT LINODE_ID")
        ...
        action_id = create_pending_action(update.effective_user.id, "delete", [target], {"source": "cli"})
        audit_event("linode.delete.requested", "requested", update=update, account=account, resource={"type": "linode", "id": linode_id})
        await update.message.reply_html(pending_action_text(action_id), reply_markup=pending_action_keyboard(action_id))
    except Exception as e:
        await update.message.reply_text(f"Error: {e}")
...
async def types_text(force: bool = False) -> str:
    ...
    for t in types[:100]:
        price = t.get("price") or {}
        lines.append(f"<code>{esc(t.get('id'))}</code> - {esc(t.get('label'))} ${esc(price.get('monthly', '-'))}/mo ${esc(price.get('hourly', '-'))}/hr")
    return "\n".join(lines)
...
async def refresh_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        audit_event("catalog.refresh", "success", update=update, request={"regions": len(regions), "types": len(types), "images": len(images)})
...
async def group_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    parts = (update.message.text or "").split()
    if len(parts) < 3:
        await update.message.reply_text("Usage: /group create|list|delete|reboot|export NAME [all|ACCOUNT] [key=value]")
        return
    sub = parts[1].lower()
    ...
    if sub == "create":
        ...
        plan = await finalize_plan(build_plan_from_options(args, update.effective_user.id))
        PENDING_CREATES[update.effective_user.id] = plan
        await update.message.reply_html(plan_summary(plan), reply_markup=confirm_keyboard())
        return
    ...
    if sub == "list":
        ...
        for p in parts[3:]:
            if p in {"txt", "json", "wa", "tg"}:
                fmt = p
        await send_export(update.message, await collect_group_targets(name, scope), fmt, f"group-{name}", include_secret=False)
    elif sub in {"delete", "reboot"}:
        action_id = create_pending_action(update.effective_user.id, sub, targets, {"group": name, "scope": scope})
        PENDING_CONFIRM_TEXT[update.effective_user.id] = action_id if action_confirm_phrase(sub, len(targets)) else ""
        await update.message.reply_html(pending_action_text(action_id), reply_markup=pending_action_keyboard(action_id))
    else:
...
async def text_input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid in PENDING_CONFIRM_TEXT:
        ...
        phrase = action_confirm_phrase(p.get("action"), len(p.get("targets", []))) if p else None
        value = (update.message.text or "").strip()
        if phrase and value == phrase:
            await execute_pending_action(uid, action_id, update=update)
        else:
            await update.message.reply_text(f"Confirm salah. Ketik {phrase} atau /cancel.")
        return
    uid = update.effective_user.id
    ...
    if mode == "label":
        ...
        state["group"] = sanitize_group_name(value) if value.lower() not in {"clear", "none", "-"} else ""
    elif mode == "root_pass":
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if data == "confirm_create":
            ...
            await execute_pending_action(uid, data.split(":", 2)[2], update=update, message=q.message)
            ...
            await safe_edit_or_reply(update, "<b>Linode bot ready.</b>", main_keyboard())
            ...
            await send_export(q.message, await collect_instances("all"), "txt", "all-vps", include_secret=False)
            ...
            reload_accounts(); CATALOG_CACHE.clear(); ACCOUNT_HEALTH_CACHE.clear()
            regions, types, images = await asyncio.gather(regions_catalog(True), types_catalog(True), images_catalog(True))
            audit_event("catalog.refresh", "success", update=update, request={"regions": len(regions), "types": len(types), "images": len(images)})
            ...
            WIZARDS.pop(uid, None); WIZARD_INPUT.pop(uid, None)
            ...
            state = get_wizard(uid)
            if field == "account":
                get_account(value); state["account"] = value; state["account_mode"] = "specific"; state["smart_create"] = False
            elif field == "account_mode":
                if value not in {"random", "roundrobin", "spread", "smart"}:
                    raise BotError("account mode invalid")
                state["account_mode"] = value; state["account"] = state.get("account") or default_account_name(); state["smart_create"] = value == "smart"
            elif field == "region":
                if value == "random":
                    state["random_region"] = True; state["region_mode"] = "random"; state["region"] = DEFAULT_REGION
                elif value == "auto":
                    state["random_region"] = True; state["region_mode"] = "auto"; state["region"] = DEFAULT_REGION; state["smart_create"] = True
                else:
                    state["random_region"] = False; state["region_mode"] = "specific"; state["region"] = value
            elif field == "type":
                state["type"] = value
            elif field == "image":
                state["image"] = value
            ...
            state = get_wizard(uid)
            if field == "random":
                state["random_region"] = not bool(state.get("random_region")); state["region_mode"] = "random" if state["random_region"] else "specific"
            elif field == "smart":
                state["smart_create"] = not bool(state.get("smart_create"));
                if state["smart_create"]: state["account_mode"] = "smart"
            elif field == "backups":
            ...
            await safe_edit_or_reply(update, prompts.get(mode, "Kirim value."), InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Menu", callback_data="wiz:menu")]]))
        elif data == "dash:menu":
            await render_dashboard(update, context)
        elif data in {"dash:prev", "dash:next"}:
            st = get_dashboard(uid); st["page"] += -1 if data.endswith("prev") else 1
            await render_dashboard(update, context)
        elif data.startswith("dash:toggle:"):
            idx = int(data.split(":")[-1]); st = get_dashboard(uid); item = st.get("page_items", [])[idx]; key = row_key(item); selected = st.setdefault("selected", set())
            selected.remove(key) if key in selected else selected.add(key)
            await render_dashboard(update, context)
        elif data.startswith("dash:del:") or data.startswith("dash:reboot:"):
            parts = data.split(":"); action = "delete" if parts[1] == "del" else "reboot"; idx = int(parts[2]); item = get_dashboard(uid).get("page_items", [])[idx]
            action_id = create_pending_action(uid, action, make_targets([item]), {"source": "dashboard"})
            await q.message.reply_html(pending_action_text(action_id), reply_markup=pending_action_keyboard(action_id))
        elif data == "dash:selectpage":
            st = get_dashboard(uid); selected = st.setdefault("selected", set())
            for item in st.get("page_items", []): selected.add(row_key(item))
            await render_dashboard(update, context)
        elif data == "dash:clear":
            get_dashboard(uid)["selected"] = set(); await render_dashboard(update, context)
        elif data.startswith("dash:filter:"):
            st = get_dashboard(uid); field = data.split(":")[-1]
            if field == "account":
                choices = ["all"] + [a["name"] for a in enabled_accounts()]
                st["account"] = choices[(choices.index(st.get("account", "all")) + 1) % len(choices)] if st.get("account", "all") in choices else "all"
            elif field == "status":
                choices = ["all", "running", "offline", "booting", "provisioning", "rebooting", "deleting", "error"]
                st["status"] = choices[(choices.index(st.get("status", "all")) + 1) % len(choices)] if st.get("status", "all") in choices else "all"
            st["page"] = 0; await render_dashboard(update, context)
        elif data == "dash:ask:region":
            WIZARD_INPUT[uid] = "dash_region"; await q.message.reply_html("Kirim region filter, contoh <code>id-cgk</code> atau <code>all</code>.")
        elif data == "dash:ask:search":
            WIZARD_INPUT[uid] = "dash_search"; await q.message.reply_html("Kirim search label/id/ip, atau <code>clear</code>.")
            ...
            rows = all_rows if data.startswith("dash:all:") else [r for r in all_rows if row_key(r) in selected]
            if not rows:
                await q.message.reply_text("No selected targets."); return
            action = data.split(":")[-1]
            targets = make_targets(rows)
            if action == "export":
                await send_export(q.message, rows, "txt", "selected-vps", include_secret=False); return
            if action == "resize":
                PENDING_RESIZE_TARGETS[uid] = targets; WIZARD_INPUT[uid] = "resize_type"; await q.message.reply_html("Kirim type baru. Contoh: <code>g6-standard-2</code>"); return
            ...
            fmt = data.split(":")[-1]
            await send_export(q.message, LAST_CREATED.get(uid, []), fmt, "new-vps", include_secret=True)
    except Exception as e:
